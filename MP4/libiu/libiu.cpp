...
static struct bpf_map *iu_object__add_map(struct bpf_object *obj)
{
	...
	new_cap = (obj->maps_cap * 3 / 2) > (size_t)4 ? obj->maps_cap * 3 / 2 : (size_t)4;
...
static int iu_object__init_prog(struct bpf_object *obj, struct bpf_program *prog,
		      const char *name, size_t sec_idx, const char *sec_name,
		      size_t sec_off, void *insn_data, size_t insn_data_sz, const struct bpf_sec_def *sec_def)
{
...
int iu_obj::parse_scns(struct bpf_object *obj)
{
	...
	if (!gelf_getehdr(obj->efile.elf, &obj->efile.ehdr)) {
		std::cerr << "elf: failed to get ELF header from " << obj->path << elf_errmsg(-1) << std::endl;
		return -1;
	}

	if (elf_getshdrstrndx(obj->efile.elf, &obj->efile.shstrndx)) {
		std::cerr << "elf: failed to get section names section index for " << obj->path << elf_errmsg(-1) << std::endl;
		return -1;
	}
	...
	for (auto scn = elf_nextscn(elf, NULL); scn; scn = elf_nextscn(elf, scn)) {
		...
		if (sh->sh_type == SHT_SYMTAB){
			...
			if (!data){
				std::cerr << "elf: failed to get section(" << idx << ") data from " << obj->path << ": " << elf_errmsg(-1) << std::endl;
				return -1;
			}
		...
		// Except symtab and maps, other indexes are saved only for checks in some libbpf functions
...
int iu_obj::parse_progs(struct bpf_object *obj)
{
	...
	for (int i = 0; i < nr_syms; i++) {
		...
		if (!bpf_progs){
			std::cerr << "sec "<< scn_name << ": failed to alloc memory for new program "<< sym_name << std::endl;
			return -ENOMEM;
		}
		obj->programs = bpf_progs;
		bpf_prog = &bpf_progs[nr_progs];

		if (iu_object__init_prog(obj, bpf_prog, sym_name, elf_ndxscn(scn), scn_name, sym->st_value, (char *)data->d_buf + sym->st_value, sym->st_size, sec_def))
			return -1;
...
struct bpf_object *iu_object__open(char *path)
{
	...
	if (!path)
		return NULL;
	obj = (struct bpf_object *) calloc(1, sizeof(struct bpf_object) + strlen(path) + 1);
	if (!obj) {
